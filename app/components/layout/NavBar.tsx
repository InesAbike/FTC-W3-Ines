...
const Navbar: React.FC = () => {
  ...
  // Empêche le défilement du corps de la page quand le menu mobile est ouvert
  useEffect(() => {
    if (isMobileMenuOpen) {
  ...
  return (
    <nav className="max-w-7xl mx-auto relative">
        ...
        <div className="flex items-center justify-between gap-16 h-16">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="hidden md:inline-flex items-center px-6 py-2 space-x-2 bg-primary-dark-blue text-white rounded-full font-medium hover:bg-primary-dark-blue/90 transition-colors text-sm">
                Join the community
              </button>
              ...
              <div className="relative lg:block hidden">
                <button
                  ...
                  className="flex items-center gap-1 text-primary-dark-blue hover:text-primary-dark-blue/80 transition-colors"
                >
                  <Image
                ...
                {isLanguageDropdownOpen && (
                  <div className="absolute right-0 mt-2 w-32 bg-white rounded-md shadow-lg border border-neutral-20 py-1 z-50">
                    <button
                      ...
                      className={`flex items-center space-x-2 px-3 py-2 text-sm w-full text-left hover:bg-neutral-10 transition-colors ${currency === 'VND' ? 'bg-neutral-10 text-primary-dark-blue' : 'text-neutral-80'
                        }`}
                    >
                    ...
                    <button
                      ...
                      className={`flex items-center space-x-2 px-3 py-2 text-sm w-full text-left hover:bg-neutral-10 transition-colors ${currency === 'USD' ? 'bg-neutral-10 text-primary-dark-blue' : 'text-neutral-80'
                        }`}
                    >
                      <span className="text-xs">🇺🇸</span>
                      <span>USD</span>
                    ...
                    <button
                      ...
                      className={`flex items-center space-x-2 px-3 py-2 text-sm w-full text-left hover:bg-neutral-10 transition-colors ${currency === 'EUR' ? 'bg-neutral-10 text-primary-dark-blue' : 'text-neutral-80'
                        }`}
                    >
                      <span className="text-xs">🇪🇺</span>
                      <span>EUR</span>
      ...
      {/* Mobile menu */}
      <div className={`fixed inset-0 z-40 bg-gradient-to-r from-[#FCEED5] via-[#FCEED5] to-[#FFE7BA] transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:hidden`}>
              ...
              <div className="grid grid-cols-3 gap-2 mt-2">
                {['VND', 'USD', 'EUR'].map((curr) => (
                  <button
                    key={curr}
                    onClick={() => {
                      handleCurrencyChange(curr as Currency);
                      toggleMobileMenu();
                    }}
                    className={`flex items-center justify-center space-x-2 py-2 px-3 rounded-lg text-sm font-medium ${currency === curr ? 'bg-primary-dark-blue text-white' : 'bg-neutral-10 text-neutral-800 hover:bg-neutral-20'}`}
                  >
                    <Image
                      src={getCurrencyData(curr as Currency).flag}
                      alt={curr}
                      width={16}
                      height={16}
                      className="w-4 h-4 rounded-full"
